...
class NavigationState
{
	...
	{
		if (_isEmpty(historyBranchId))
			historyBranchId = MAIN_HISTORY_BRANCH_ID
...
const actions = {
	...
	getCurrentControl(navigationId)
	{
		...
		if (currentLevel?.isNested && Array.isArray(currentControl?.nestedControls))
		{
			...
			if (typeof lastNested === 'object')
				currentControl = lastNested
		}
	...
	{
		this.navigation.getHistory(navigationId).storeContainerState({ key, formInfo, fieldId, containerState })
	},
