...
namespace GenioMVC.Controllers
{
	public partial class PlayrController : ControllerBase
	{
		...
		private static readonly NavigationLocation ACTION_AJF_MENU_611 = new NavigationLocation("PLAYERS11892", "AJF_Menu_611", "Playr") { vueRouteName = "menu-AJF_611" };
...
// USE /[MANUAL AJF MENU_GET 21]/
			...
			if (querystring["ExportList"] != null && Convert.ToBoolean(querystring["ExportList"]) && querystring["ExportType"] != null)
			{
				...
				string file = "AJF_Menu_21_" + DateTime.Now.ToString("ddMMyyyyhhmmss") + "." + exportType;
				...
				byte[] fileBytes = null;
// USE /[MANUAL AJF OVERRQEXPORT 21]/
				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportList(listing, conditions, columns, exportType, file,ACTION_AJF_MENU_21.Name);
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "AJF_Menu_21_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_AJF_MENU_21.Name);
		...
		//
		// POST: /Playr/AJF_Menu_21_UploadFile
		[HttpPost]
		public ActionResult AJF_Menu_21_UploadFile(string importType, string qqfile) {
			AJF_Menu_21_ViewModel model = new AJF_Menu_21_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAplayr> rows = new List<CSGenioAplayr>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAplayr>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAplayr importRow in rows)
				{
					try
					{
						lineNumber++;
