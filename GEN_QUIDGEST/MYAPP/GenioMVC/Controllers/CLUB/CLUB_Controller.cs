...
namespace GenioMVC.Controllers
{
	public partial class ClubController : ControllerBase
	{
		...
		public ClubController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL AJF CONTROLLER_NAVIGATION CLUB]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAclub>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL AJF MANUAL_CONTROLLER CLUB]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Club row = new Models.Club(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("club");
			...
			try
			{
				...
				{
					case "F_CLUB__CNTRYCOUNTRY_":	// Field (DB)
						{
							var model = new F_club_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_club__cntrycountry_(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_CLUB__CNTRYCOUNTRY_":	// Field (DB)
						values = new F_club_ViewModel(UserContext.Current).GetDependant_F_clubTableCntryCountry(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_club" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_club([FromBody]F_club_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "club",
				(primaryKey) => Models.Club.Find(primaryKey, UserContext.Current, "FF_CLUB"),
				(model) => formData.MapToModel(model as Models.Club)
			);
		}
		...
		{
			return base.GetDocumsTickets("CLUB", requestModel.FieldName, requestModel.KeyValue);
		}
