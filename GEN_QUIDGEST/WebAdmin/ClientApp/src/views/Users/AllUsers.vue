<template>
	...
	<qtable 
		...
		<template #AJF="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'AJF' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.AJF.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.AJF.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.AJF.Description),
					name: "AJF",
					slot_name: "AJF",
					sort: false,
					visibility: vm.Modules.AJF.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
